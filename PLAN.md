...
## 10. 난이도 사다리 (frontier 탐색, 한 줄기 단계적)

> 통찰: 가장 가치 있는 노트는 약한 모델의 헛발질이 아니라 **강한 모델(31B)의 한계
> 에서 나온다.** 난이도를 한 줄기로 올려 31B가 strain하는 칸을 찾고 거기서 노트 수확.

- **두 축**: (a) CLI 내 복잡도↑ (파일·계약·상태·회귀) (b) 형태 확장 CLI→라이브러리→
  웹앱→GUI. 형태 확장은 게이트 비용 큼(웹=외부패키지·HTTP체크, GUI=검증난) → **킵**.
- **척추 = 전투/RPG 줄기 한 줄기 단계적**(확정): L3 자동전투 → L4 파티전투(상태이상·
  턴순서) → L5 세이브 로그라이크(절차생성+저장/로드+회귀). 도메인 고정·복잡도만↑.
- 관측: 31B가 헤매는 건 레벨 숫자보다 **상태·확률 로직**(밤 런서 미니상점·아이템강화
  실패) → 사다리에 stateful_io·persistence를 복잡도 손잡이로 강조.
- magnitude는 데이터 보고 보정(31B가 순항하면 더 크게).
