...
1. **thinking ON / LOW / OFF** — gemma 4는 thinking을 시스템 프롬프트로 제어 가능
   (`<|think|>` 토큰, LOW는 SI로 ~20%↓). 실측상 thinking이 출력의 75~88%를 먹고
   분산성 잘림(MAX_TOKENS)의 원인. **26B 구현 콜에 적용**해 품질·비용·잘림률 비교.
   31B 설계/비평은 추론이 핵심이라 ON 유지가 기본 가설. (probe로 ON 천장→LOW→OFF 순,
   OFF는 thoughts 토큰이 진짜 0 되는지 검증부터.)
