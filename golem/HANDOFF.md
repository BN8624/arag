...
2. **지금 할 일 (한 줄)**: **캠페인 결과를 부품별 신뢰도 지도로 판독 → 로그라이크 부품0 발주 준비.**
   `golem/campaign_ledger.jsonl`(주제당 1줄) 확인 → 끝났으면(10 slug) 합의율 표로 정리, 진행 중이면
   파일로 진척 보고. 끝났으면 다음 = **턴제 로그라이크 부품0(격자+결정적 이동) 발주**(★키, go 필요).
   (★새 세션은 백그라운드 프로세스를 직접 추적 못 함 — 파일로 상태 확인.)
...
3. **타워디펜스 0/11 후속(선택)** — frontier 신호. 부품 분해(경로/웨이브/타워 따로)로 gemma가 되는지,
   아니면 Claude가 채워야 하는 자리인지 = 부품-조립 모델의 첫 실전 검증 후보.
