...
2. **지금 할 일 (한 줄)**: **부품5 발주(★키, go 필요) — 모듈 베이스 위 큰 걸음.** 재설계+부품4 검증 끝(아래).
   발주 = `bank_add_roguelike_p5.py`(정밀규칙+트레이스+레퍼런스 self채점) → `driver --card rogue-p5 --base
   rogue-p4`(★키) → promote → `demo_part.py`. 파일별 생성·모듈화 검증됐으니 그대로 쌓으면 된다.
...
4. **다음 = 부품5(★키, go 필요)** — `--base rogue-p4` 위 큰 걸음(예: 다중 적, 다층 진행, 적 종류). 같은 절차.
