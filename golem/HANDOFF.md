...
2. **지금 할 일 (한 줄)**: **로그라이크 부품2(전투) 발주.** 부품0·1(던전+이동, 적+추격) 둘 다
   11/11 통과·베이스 승격 완료. 다음 = `bank_add_roguelike_p2.py`로 인접공격·HP·사망 카드 작성
   (정밀규칙+워크드트레이스) → `driver --card rogue-p2 --base rogue-p1`. ★키 런이라 사용자 go 필요.
...
1. **부품2 발주 = 전투(★키, go 필요)** — `--base rogue-p1` 위에 인접 시 공격·HP·사망 추가.
...
2. **매 부품마다 "Claude가 뭘 했나" 기록**(목표② 개입 깎기) — 반복되는 발주/조립/갭메움을 템플릿·
