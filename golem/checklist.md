...
### Phase 4 (제안, 사용자 2026-06-16) — 자율 오라클: gemma가 골든까지 만든다
지금은 Claude가 오라클(레퍼런스+규칙+시나리오)을 짠다. 사용자 제안 = 주제+골든 최소규격만 주고
31B가 오라클 설계. golem 목적(Claude 절감)에 부합. 핵심 리스크 = **오라클 신뢰**(출제=응시 같은
모델 → 상관된 오류·가짜합격). 해결안 = 오라클도 게이트: 독립 gemma K개가 *규칙만* 보고 재구현 →
다수가 골든 합의하면 "규칙이 정답을 명확히 담음"(레퍼런스 자의성 배제) + 그게 곧 통과본. 불일치
많으면 31B 규칙 재설계. Claude=메타규격+가벼운 감수. (= select-best를 오라클 검증에도 적용)
