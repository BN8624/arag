...
## 다음 액션 (★다음 세션 여기부터)
1. **조립 카드 T-000012 저항 검증 실행**: `python select_run.py`(31단독 cold, cap8). 카드 제작·검수·
   확장 완료(`bank_cards_assembly.py`). goal=PAMPHLET 전체+4시나리오 고정파티+골든 자체포함,
   오라클=A(winner/turns/엔티티별 final_hp 정확일치). **cap 안에 못 깨면 드디어 진짜 frontier.**
   장부: runs/select_ledger.jsonl.
   - 검수 반영(2026-06-15): ①HP 0클램프 goal에 명시(오버킬 12회 → 클램프없으면 음수=가짜FAIL).
     ②확장B — freeze/poison/shock 부여 스킬을 레퍼런스에 추가, 시드 스캔으로 매트릭스·DoT·감전·
     빙결스킵이 *실제 골든에서 발동*하는 시드(scenario 1/2/3/4 = seed 53/3/0/45) 선정(10현상 커버,
     승자·크기 다양). ③안 걸리는 문구(이월2회·spd동률) 정리, '행동마다 재평가+사망준비자 행동안함' 명시.
   - 골든은 `game/main.py --seed {53,3,0,45}`(또는 `--trace`)으로 재생성. 레퍼런스 test 14/14.
