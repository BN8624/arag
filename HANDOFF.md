...
2. **지금 할 일 (한 줄)**: **시니어+주니어 측정** — 26주니어/31시니어 조합이 select-best 31단독
   바닥선(T-12 ≈1/3, 한 웨이브)보다 *싸게* 통합을 깨나. 상세는 `다음 액션` 1번.
   (배경: 폴백 구현 완료. 설계 스왑 완료 — frontier=통합구현, 설계무관.)
...
## 다음 액션 (★다음 세션 여기부터)
1. **시니어+주니어(26주니어/31시니어)**: select-best 31단독 바닥선(T-12 ≈1/3, 한 웨이브)보다
   *싸게* 통합 깨나 측정. 31이 설계/비평, 26이 구현 → 비용(콜·환산$) 대비 통과율 비교.
   - 구성: GENERATOR=26B, CRITIC=31B(=#4 홈). 같은 frozen 설계·오라클로 select-best 돌리면
     바닥선과 동일조건 비교. select_run env만 바꾸면 됨(현재 31단독 하드코딩 → 분기 필요).
2. **warm 캠페인**(맨 뒤): 누적 노트로 cold 대비 델타.
