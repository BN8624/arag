...
## 돌고 있는 것
- **31단독 50런(obs_run.py)** — cold, 8장 라운드로빈, 통짜/분할 교대, 단발. 진행 ~36/50,
  ~07시 KST 완료 예상. 대시보드(http://100.89.73.83:8400, 31단독만 필터됨)로 실시간.

## 로드맵 (결정 19 — CLI 두 층 + 다음)
**층1(능력매핑)은 warm까지면 끝. 층2(오케스트레이션)가 ARAG 본론, 거의 미답.**

1. **50런 분석** (끝나는 대로): 카드별 통과율·분산(천장 vs 분산 지도) + **harness_lift**
   (자가수정이 건진 비율) + **T-7 왜 0/N — 모델 한계냐 하네스/오라클 버그냐**(과거 최대산출이
   늘 하네스버그였음, 단정 전 events 확인) + T-5 약점 + 통짜vs분할 최종.
2. **결정18 index 정리**: 옛 entry prune(캠페인 끝나 레이스 없음). 대시보드 임시필터 → 영구.
3. **31단독 warm 캠페인** = *진짜 안 끝난 핵심 실험*. cold 50런이 노트 축적 → 같은 8카드 warm
   N회 → cold vs warm 델타. 실패카드(T-5·T-7)를 warm이 구하나가 가치 증명. (함정: 검색 관련성,
   self-fix 중복 → warm 표적 좁음.)
4. **층2 — 오케스트레이션을 T-7에 붙여 frontier 깨기**: select-best/빔, 시니어+주니어(#3),
   국소패치. 단일패스 0/N인 T-7을 통과시키면 thesis 증명, 못 하면 진짜 천장. ← CLI 마지막 핵심.
5. **형태 확장(별도 축)**: 어댑터 → 2D **방치형**(베스트핏). 행동=gemma+오라클, 에셋=나중 바인딩.
6. **폴백 구현(보류 가능)**: `llm.py generate()` 26B 죽으면 31B 1회 재시도. 26B 손 쓰는 구성 재가동 시 필요.
