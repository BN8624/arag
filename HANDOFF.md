...
2. **지금 할 일 (한 줄)**: **폴백 구현** — `llm.py` generator(26B) 콜이 5xx/429로 재시도 소진 시
   31B로 1회 강등(26단독 arm은 no-op), 테스트 추가. 상세는 `다음 액션` 1번.
   (배경: 설계 스왑 완료 — A·B 둘 다 ≈1/3 → frontier=통합구현, 설계무관. 다음은 가용성/비용.)
...
## 다음 액션 (★다음 세션 여기부터)
1. **폴백 구현(가용성)**: `llm.py` generator(손=26B) 콜이 5xx/429로 재시도 소진 시 31B로 1회 강등.
   - **26단독 no-op**: 폴백 대상(31B)이 generator와 같으면(=이미 31B거나 26단독 arm) 강등 안 함.
   - 5xx/429에만(다른 에러는 그대로 raise). 강등 콜도 페이서/계측 통과. 테스트(주입 클라로 5xx강제).
2. **시니어+주니어(26주니어/31시니어)**: select-best 바닥선보다 *싸게* 통합 깨나 측정.
