...
2. **지금 할 일 (한 줄)**: **설계 스왑으로 frontier 귀속** — frozen/T-000012 외 다른 고정설계
   2~3개를 만들어 같은 병렬 select-best로 재측정, 통과율이 설계에 좌우되는지 본다.
   (배경: T-12는 이미 병렬 select-best로 깸. 단일시도 ≈1/3 → 통합은 진짜 어렵지만 벽은 아님.)
...
## 다음 액션 (★다음 세션 여기부터)
1. **설계 스왑으로 frontier 귀속(설계 vs 구현)**: T-12는 frozen/T-000012(140607 설계)로 ≈1/3 통과.
   이제 다른 고정설계 2~3개를 만들어(`bank_cards_assembly.py` goal로 fresh 설계 N개 뽑아 static·
   stdlib 통과분을 `frozen/T-000012-b`, `-c` 등으로 박제) 같은 병렬 select-best 재측정.
   - 설계마다 통과율이 크게 갈리면 frontier=설계, 다 비슷하게 ~1/3이면 frontier=통합구현.
   - 실행: `python select_run.py`(FROZEN_DIR/tid 가리킴 — 스왑하려면 CARDS나 FROZEN 경로 조정).
   - ⚠️ 실제 API. 31단독이라 시간대 무관(500은 서버 시변이라 오후 피하면 유리하나 백오프가 흡수).
2. **확정 후** → **폴백 구현**(`llm.py` 26B 5xx/429 소진 시 31B 1회 강등, 26단독 no-op, 테스트)
   → **시니어+주니어(26주니어/31시니어)**: select-best 바닥선보다 *싸게* 통합 깨나.
