...
2. **지금 할 일 (한 줄)**: **전31 재측정** — 병렬 인프라(키풀·연속리필·도커캡, checklist 1~3) 완료됨.
   안정 시간대에 `python select_run.py T-000012 31solo <width>` 실행 = 11키 병렬 신뢰 베이스라인.
   첫 병렬 실행에서 **429 0건**으로 쿼터 독립 확인. 상세 = `다음 액션` 1번.
...
- 4home(26손/31머리) 1키 측정은 **중단됨**(피크시간 0/3, 사용자 결정). 부분데이터만 남음 —
  완료 sel01~03은 각 26B 6~8콜+31B 1콜 후 exec게이트 abort, 폴백 강등 1콜뿐(경미). 결론 불가.

## 다음 액션 (★다음 세션 여기부터)
1. **전31 재측정 (인프라 위에서, 안정 시간대)**: arm=31solo로 select-best 재실행 = 신뢰 베이스라인.
   - 도구: `python select_run.py T-000012 31solo <width> [n_docker]`. 인자 = frozen / arm /
     width(동시워커, 기본=min(키수,CAP)) / n_docker(동시컨테이너, 기본 2). 장부 select_ledger.jsonl.
   - **첫 병렬 실행에서 429 0건 확인 = 키별 쿼터 독립 검증**(아니면 키풀크기 낮춰야).
   - 31solo는 26B 미사용이라 26 사망시간 영향 없음(아무 때나 가능). (선택) 4home은 안정시간대에만.
   - width 작게=비용측정, 크게=throughput/variance. 같은 frozen/T-000012+오라클 고정.
2. **(선택) 4home 깨끗이 재측정**: 안정 시간대 `select_run.py T-000012 4home <width>` → 비용↔통과율 비교(원목표).
3. **warm 캠페인**(맨 뒤): 누적 노트로 cold 대비 델타.
