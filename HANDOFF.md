...
2. **지금 할 일 (한 줄)**: 운영 구성 = **31solo 단독**(결정25). 병렬 인프라·RPD·대시보드·베이스라인·
   전수검토 완료. 다음 = **warm 캠페인(31solo)**. 상세 = `다음 액션`.
...
## 다음 액션 (★다음 세션 여기부터)
> 운영 구성 = **31solo 단독**(결정25). 비용·역할비교 측정은 제안·재개 금지 — 사용자가 먼저 꺼낼 때만.
1. **warm 캠페인 (31solo)**: 누적 노트(lessons/critique_notes)로 cold 대비 델타(노트 효과 정식 측정).
2. **형태 확장(별도 축)**: 어댑터 → 2D 방치형(game/ 정본 재사용).
3. temperature 고정값(추천 0.2) + gemma temperature/seed 지원 여부 확인.
