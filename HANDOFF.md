...
2. **지금 할 일 (한 줄)**: **병렬 인프라 구축** — `checklist.md` 1번부터(키풀+키별페이서 → 연속리필
   → 도커캡). 11키 워커풀로 진짜 병렬 만든 뒤, 그 위에서 전31 재측정. 상세는 `다음 액션` 1번.
   (배경: 4home 1키 측정을 피크시간에 돌려 0/3+중단 → 사용자 결정으로 인프라 먼저. 결정22.)
...
## 다음 액션 (★다음 세션 여기부터)
1. **병렬 인프라 구축 (checklist.md 따라, 결정22)**: 11키 워커풀로 진짜 병렬. 순서 = ①키풀+키별
   페이서(llm.py) → ②연속리필 풀(select_run.py, 웨이브배리어 제거) → ③도커 세마포어+`--memory`캡
   (docker_gate.py). 설계 = **워커=키**(시도 하나가 키 하나 통째로, 26·31 다 그 키로). 노브 둘 분리
   — 키풀크기(쿼터상한 ≤11) / 도커세마포어(PC RAM, 4부터 실측). 인프라 동안 측정 금지(변수오염).
2. **전31 재측정 (인프라 위에서, 안정 시간대)**: arm=31solo로 select-best 재실행 = 신뢰 베이스라인.
   같은 frozen/T-000012+오라클, width 올려 벽시계 단축. (선택) 4home도 안정시간 깨끗이 재측정 → 비용비교.
   - 도구: `python select_run.py <frozen> <arm>` (arm=31solo|4home, 이미 분기 추가됨). 장부 select_ledger.jsonl.
