# Progress: 자가개선 인프라(0~3층) + 대시보드 완료

## 현황 요약 (2026-06-11 갱신, 5차 세션)
- **상태**: 테스트 151개 통과. 3층(자동 출제기 + 배치 모드) 구현·연결 완료
- **대시보드 가동 중**: 시작프로그램 등록, 테일스케일 IP로 폰 접속, 로그 한글(공장 비유)
  - 모드 버튼: 단일(작동) / 개선(작동) / **자동(작동 — 회차 수 1~20 입력)** / 종료예약(작동)
- 출제기 실전 검증: --dry 2회(수집·추첨 OK) + 실제 1콜(31B가 한국어 아이디어 정상 출제)

## 바로 다음 작업 (우선순위 순)

1. **배치 모드 실전 가동**: 대시보드 "자동" 버튼으로 2~3회차 돌려보기
   (한 회차 30~60분 + 콜 ~30회 — 쿼터 여유 있는 날, 폰에서 버튼만 누르면 됨)
   - 돌린 뒤 볼 것: 적응형 난이도가 움직이나(auto_state.json level),
     중복 출제 없나, 재발률 표시(헤더 "재발 m/n")가 채워지나
