...
1. **sop급 본게임 검증 런**: "PDF에서 텍스트 추출 → 모의 AI API 호출 → 엑셀 저장"
   같은 sop 축소판 아이디어로 E2E 1회. pypdf·openpyxl·모킹 패턴이 한 런에서
   같이 시험됨. 통과하면 D-2(외부 API 앱 지원) 목표 달성 선언.
   - 실행: `python orchestrator.py "<아이디어>" --max-minutes 40`
2. **LGTM 유도 효과 관찰**: 만점 빌드에 비평가가 여전히 revise를 내는지.
   여전히 통과 빌드를 깎으면 다음 처방은 "비평 반영 전 점수 예측" 또는
   "만점 시 비평 라운드 자체를 생략"하는 구조적 차단.
3. (선택) 토큰카운트(로드맵 C) — usage 필드 파싱은 llm.py 한 곳 수정이라 작음.
